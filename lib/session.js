...
async function isAdmin() {
	...
	const result = session.data.user.privileges.some(role => role.toLowerCase() === 'ek_admin');
...
async function hasPrivilege(privilege) {
	...
	const hasAdminRole = session.data.user.privileges.some(role => role.toLowerCase() === 'ek_admin');
