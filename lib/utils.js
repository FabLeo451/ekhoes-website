...
function isAdmin(session) {
  ...
  if (!session.user.hasOwnProperty('privileges'))
    return false;

  const result = session.user.privileges.some(role => role.toLowerCase() === 'admin');
...
function hasPrivilege(session, privilege) {
  ...
  const included = session.user.privileges.some(role => role.toLowerCase() === privilege.toLowerCase());

  return included || isAdmin(session);
}
*/
...
export { detectBrowser, detectPlatform, getSystemMetrics };
